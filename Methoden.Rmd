...
_Shapiro-Wilk Test:_ Mithilfe des Shapiro-Wilk Tests für Normalität wird der Datensatz auf Normalverteilung getestet. Bei errechneten P-Werten von $\geq 5*10^{-4}$ liegt keine Normalverteilung mehr vor und der Datensatz wurde als negative Amplifikation gewertet.  
_Residuen Wachstums Test:_ Bei diesen Test wird die Stabilität der Residuen in der linearen Phase des Anstiegs untersucht. Bei negativen Amplifikationen weichen die Fluoreszenzwerte von dem linearen Modell ab. Dadurch korrelieren die Residuen im Gegensatz zu positiven Amplifikationen stark mit den Fluoreszenzwerten. Anhand eines Schwellenwertes von 0.5 wurde diese Korrelation Untersucht und der Datensatz als positive/negative Amplifikation eingestuft.  
...
_Signal Level Test:_ Dieser Test vergleicht zwei aus dem Datensatz berechnete Werte. Der Erste berechnet sich aus der Formel \@ref(eq:eins). Dabei steht MAD^[https://search.r-project.org/CRAN/refmans/ie2misc/html/madstat.html] (engl. _Mean-absolut deviation_) für die absolute Standardabweichung, siehe R Dokumentation.  Der zweite Wert ist das Signal-Rausch-Verhähltnis (SNR), berechnet mit der Formel \@ref(eq:zwei). Bei einem um 25 % erhöhtem Wert des SNR im Vergleich zum ersten Wert wurde die Amplifikation als positiv gewertet.  
...
_Schwellenwert Test:_ Bei DNA-Amplifikationsmethoden ist es üblich, eine Amplifikation als negativ einzustufen, sollte ein gewisser Schwellenwert nicht überschritten werden [@Aranha2021]. Für die Berechnung des Schwellenwertes wurde die von @Frey1998 beschriebene Methode verwendet. Diese beruht auf dem Median der gesamten Daten zu welchem eine Faktor addiert wird. Der Faktor bildet sich dabei aus der Standardabweichung (SD) der Daten multipliziert mit einem Term bestehend aus der Probenanzahl (n) und den Werten einer einseitigen students t-Verteilung (siehe Formel \@ref(eq:vier)). Für die Schwellenwert-berechnung wurden mindestens 8 Negativkontrollen als Datensatz verwendet. Das Konfidenzintervall für die t-Verteilung wurde als 0.99 (99 %) festgelegt. 
...
**Ermittlung der Ansteigszeit (engl. threshold time, TT-Wert)**


   
### Probit-Analyse

Die Probit-Analyse ist ein statistisches Modell, um binäre Werte 

\textcolor{red}{bis hier hin Kontrolliert :D der andere Teil ist noch in Bearbeitung haha.}
...
kable(parameter, booktabs = TRUE,col.names = NULL, caption = "Temperaturprotokoll für die Kolonie-PCR") %>%
  ...
  add_header_above(c("Temperatur"= 1, "Zeit"= 1, "Zyklen"=1))
...
#### Influenza B PCR
\hfill
Die Ampflifikation von Influenza B Virus Nukleinsäuren mittels PCR wurde mit dem Luna\textsuperscript{\textcopyright} Universal Probe One-Step RT-qPCR Kit durchgeführt. Im ersten Schritt wurden pro Reaktion 19 µl Reaktionsmix mit DEPC-H~2~O (\textcolor{red}{Thermo Fisher Scientific}) siehe Tabelle \@ref(tab:infbPCR) hergestellt. Für den in Tabelle \@ref(tab:infbPCR) angegebene Primer-Sonden-Mix (PSM) wurde der in  Tabelle \@ref(tab:infbPSM) beschriebene Influenza B PSM verwendet. Die Primersequenzen wurden durch das nationale Zentrum für Immunisierung and Atemwegserkrankungen (U.S.) beschrieben [@ImmunizationJuly132021].
...
```{r, infbPSM}
infbPSM <- read.csv2("infbPSM.csv", header = FALSE)
kable(infbPSM, booktabs = TRUE, col.names = NULL, caption = "Zusammensetzung des Influenza B Primer-Sonden-Mixes") %>%
  add_header_above(c("Name"= 1, "Sequenz (5´->3´)"= 1, "Konzentration"=1, "Modifikation"=1)) %>%
  kable_styling(latex_options = "hold_position") %>%
  kable_styling(latex_options = "scale_down") 
```

Zu dem Reaktionsmix wurde 1 µl Influenza B Virus RNA dazugegeben und die 20 µl Gesamtmix wurden nach dem in Tabelle \@ref(tab:infbTEMP) angegeben Temperaturprotokoll im LightCycler\textsuperscript{\textcopyright} 480 (Roche Holding) im Cy5-Messkanal gemessen. Die Auswertung der Daten erfolgte mit den in \@ref(chipPCR) beschrieben statistischen Verfahren.
...
#### Influenza A PCR
\hfill
Die Ampflifikation von Influenza B Virus Nukleinsäuren mittels PCR wurde mit dem Luna\textsuperscript{\textcopyright} Universal Probe One-Step RT-qPCR Kit durchgeführt. Im ersten Schritt wurden pro Reaktion 19 µl Reaktionsmix mit DEPC-H~2~O (\textcolor{red}{Thermo Fisher Scientific}) nach dem gleichen Muster wie der Influenza B Reaktionsmix (siehe Tab. \@ref(tab:infbPCR)) hergestellt. Für den in Tabelle \@ref(tab:infbPCR) angegebene Primer-Sonden-Mix (PSM) wurde der in  Tabelle \@ref(tab:infbPSM) beschriebene Influenza A PSM verwendet. Die Primersequenzen wurden durch das nationale Zentrum für Immunisierung and Atemwegserkrankungen (U.S.) beschrieben [@ImmunizationJuly132021].

```{r, infaPSM}
infaPSM <- read.csv2("infaPSM.csv", header = FALSE)
...
\pagebreak
Zu dem Reaktionsmix wurde 1 µl Influenza A Virus RNA dazugegeben und die 20 µl Gesamtmix wurden nach dem in Tabelle \@ref(tab:infbTEMP) angegeben Temperaturprotokoll im LightCycler\textsuperscript{\textcopyright} 480 (Roche Holding) im HEX-Messkanal gemessen. Die Auswertung der Daten erfolgte mit den in \@ref(chipPCR) beschrieben statistischen Verfahren
...
kable(RPAoligo, booktabs = TRUE, col.names = NULL, caption = "Entwickelte Primer und Sonden für die Recombinase Polymerase Amplifikation") %>%
  add_header_above(c("Name"= 1, "Sequenz (5´->3´)"= 1, "Modifikation"=1)) %>%
